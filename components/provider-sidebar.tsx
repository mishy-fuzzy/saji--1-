...
export default function ProviderSidebar({
  ...
  const [jobsBadgeCount, setJobsBadgeCount] = useState(0);
  ...
  useEffect(() => {
    ...
    const loadSidebarBadges = async () => {
      try {
        ...
        const bookings = Array.isArray(bookingsPayload?.data)
          ...
          : [];
        const actionableJobs = bookings.filter((booking: any) => {
          const status = String(booking?.status || "pending").toLowerCase();
          return ["pending", "accepted", "assigned", "in-progress"].includes(
            status,
          );
        }).length;
        ...
        setJobsBadgeCount(actionableJobs);
        setMessagesBadgeCount(unreadMessages);
      } catch {
        setJobsBadgeCount(0);
    ...
    loadSidebarBadges();
    const intervalId = window.setInterval(loadSidebarBadges, 25000);
  ...
  const menuItems = useMemo(
    () => [
      { icon: Home, label: "Home", href: "/provider", exact: true },
      {
        ...
        label: "Jobs",
        href: "/provider/jobs",
        badge: jobsBadgeCount > 0 ? jobsBadgeCount : undefined,
      },
      ...
      { icon: ImageIcon, label: "My Content", href: "/provider/content" },
      {
        ...
        badge: messagesBadgeCount > 0 ? messagesBadgeCount : undefined,
      },
    ...
    [jobsBadgeCount, messagesBadgeCount],
  );
  ...
  const profileMenuItems = [
    ...
    { label: "Proof of Work", href: "/provider/profile/proof" },
  ...
  const toggleProfileMenu = useCallback(() => {
    startTransition(() =>
      setExpandedMenu((prev) => (prev === "profile" ? null : "profile")),
    );
  }, [startTransition]);
                ...
                <div key={item.href}>
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label="Toggle profile submenu"
                    >
                      <ChevronDown
