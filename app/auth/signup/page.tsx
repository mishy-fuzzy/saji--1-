...
function SignupContent() {
  ...
  const getPasswordStrength = () => {
    ...
    if (score <= 1)
      return { label: "Weak", width: "25%", color: "bg-destructive" };
    if (score === 2)
      return { label: "Fair", width: "50%", color: "bg-amber-500" };
    if (score === 3)
      return { label: "Good", width: "75%", color: "bg-emerald-500" };
          ...
          {[
            {
              ...
              desc: "Create your account in just 2 steps",
            },
            {
              icon: Shield,
            ...
            },
            {
              ...
              desc: "Start using SAJI immediately",
            },
          ].map((item) => (
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8 sm:px-8">
        <div className="w-full max-w-[420px]">
          ...
          <p className="text-sm text-muted-foreground mb-5">
            Join SAJI in 2 simple steps
          </p>

          {/* Progress */}
          <div className="flex gap-2 mb-5">
            <div
              className={`flex-1 h-1.5 rounded-full transition-colors ${step >= 1 ? "bg-primary" : "bg-muted"}`}
            />
            <div
              className={`flex-1 h-1.5 rounded-full transition-colors ${step >= 2 ? "bg-primary" : "bg-muted"}`}
            />
          </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-12 h-10 rounded-xl border-border bg-card"
                    disabled={isLoading}
                  />
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.password && (
                  <div className="mt-2">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`text-[11px] font-medium ${pwStrength.color === "bg-destructive" ? "text-destructive" : pwStrength.color === "bg-amber-500" ? "text-amber-600" : "text-emerald-600"}`}
                      >
                        {pwStrength.label}
                      </span>
                    </div>
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  disabled={isLoading}
                  variant="outline"
                  className="flex-1 h-10 rounded-xl bg-transparent gap-1"
                >
