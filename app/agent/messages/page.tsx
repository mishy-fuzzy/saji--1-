...
export default function AgentMessagesPage() {
  ...
  async function loadContacts() {
    const response = await fetch("/api/agent/users", { cache: "no-store" })
    const payload = await response.json()
    if (!response.ok || !payload?.ok) {
      ...
      <div className={`${selectedPeerId ? "hidden sm:flex" : "flex"} flex-col w-full sm:w-80 border-r border-gray-200 dark:border-gray-700`}>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
          {conversationRows.map(c=>(
            <button key={c.peerId} onClick={()=>setSelectedPeerId(c.peerId)} className={`w-full text-left px-3 py-3 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 ${selectedPeerId===c.peerId?"bg-indigo-50 dark:bg-indigo-900/20":""}`}>
              <div className="w-9 h-9 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{c.peer.name.slice(0,1)}</div>
      ...
      <div className={`${selectedPeerId?"flex":"hidden sm:flex"} flex-col flex-1`}>
        {selectedPeer?(<>
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center gap-3">
            <button onClick={()=>setSelectedPeerId(null)} className="sm:hidden text-gray-600"><ArrowLeft size={20}/></button>
            <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-bold">{selectedPeer.name.slice(0,1)}</div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.map((m)=>(<div key={m.id} className={`flex ${m.senderId===selectedPeerId?"justify-start":"justify-end"}`}><div className={`max-w-[75%] px-3 py-2 rounded-xl text-sm ${m.senderId===selectedPeerId?"bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-bl-sm":"bg-indigo-600 text-white rounded-br-sm"}`}><p>{m.text}</p><p className={`text-[9px] mt-1 ${m.senderId===selectedPeerId?"text-gray-400":"text-indigo-200"}`}>{new Date(m.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></div></div>))}
          ...
          <div className="p-3 border-t border-gray-200 dark:border-gray-700 flex gap-2">
            ...
            <button onClick={handleSend} disabled={sending} className="p-2 bg-indigo-600 rounded-lg text-white hover:bg-indigo-700 disabled:opacity-50"><Send size={16}/></button>
          </div>
