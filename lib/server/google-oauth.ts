...
function getRequiredEnv(name: string): string {
  ...
  // Keep env parsing resilient when a value contains accidental inline notes.
...
export function resolveGoogleRedirectUri(origin?: string): string {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "Missing GOOGLE_REDIRECT_URI environment variable in production",
    );
  }
...
export async function fetchGoogleUserInfo(accessToken: string) {
  ...
  if (!response.ok || !payload.email) {
    throw new Error(
      payload?.error?.message || "Failed to fetch Google user info",
    );
  }
